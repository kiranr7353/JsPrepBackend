...
exports.getSnippetsData = async (req, res) => {
    try {
        ...
        if(searchText && searchText?.length > 0) {
            titlRef = await db.collection('snippets').where('topicId', '==', topicId).where('categoryId', '==', categoryId).where('searchTerm', 'array-contains', searchText.toLowerCase()).orderBy('createdAt').limit(pageSize).offset(pageSize * (pageNumber - 1)).get();
            count = await db.collection('snippets').where('topicId', '==', topicId).where('categoryId', '==', categoryId).where('searchTerm', 'array-contains', searchText.toLowerCase()).count().get();
        } else {
            titlRef = await db.collection('snippets').where('topicId', '==', topicId).where('categoryId', '==', categoryId).orderBy('createdAt').limit(pageSize).offset(pageSize * (pageNumber - 1)).get();
